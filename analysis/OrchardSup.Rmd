...
This document is directly based on the methods used in the Founder Species
project, also a collaboration with Hans Bernstein in 2016-2017.
...
```{r meta-graphs, fig.height=7, fig.show="hide", results="hide"}
table(meta$Time..H., meta$Substrate)
...
ggsave('fig1-ab.pdf', path = "./figures/", units = "mm", width = 61, height = 122, device = cairo_pdf, scale = 1.2)
```

#### New Figure: productivity vs irradiance

Notes to Hans: This graph does not get a second panel for 16S and 18S because
only a single measurement of carbon uptake was made. This shows one of the
...
ggsave('fig-sup.pdf', path = "./figures/", units = "mm", width = 89, height = 120, device = cairo_pdf, scale = 0.9)
...
rar.18s <- rarefy_even_depth(full18s, sample.size = 22000)
rar.18s
...
These would look good as little heat maps... 
...
#tax_table(rar.16s.cohort.merged.top)[c("OTU_20", "OTU_12", "OTU_13")]

fig1.c <- plot_bar(rar.16s.cohort.merged.top, x = "Time..H.", fill = fillrank, facet_grid = ~Substrate)
fig1.c <- fig1.c +
  scale_fill_viridis(discrete = T, begin = 0, end = .9, name = "16S Taxa") +
  labs(x = "Time (h)") +
...
#tax_table(rar.18s.cohort.merged.top)[c("OTU_77", "OTU_57", "OTU_42")]

fig1.d <- plot_bar(rar.18s.cohort.merged.top, x = "Time..H.", fill = fillrank, facet_grid = ~Substrate)
fig1.d <- fig1.d +
...
ggsave('fig1-cd.pdf', path = "./figures/", units = "mm", width = 122, height = 122, device = cairo_pdf, scale = 1.3)
```

## Beta Diversity

Look for microbes that covary with carbon uptake (using the bi-plot)

Because I'm not confident of the intricacies of building ML trees from any MSA
...
# I'm not sure that ADONIS handles timed data well. The strata= argument could
# help, but I need feedback on this.
...
alpha.16s.alt <- alpha.16s.alt + 
  ...
  geom_jitter(width = 0.1) +
  facet_grid(facets = variable~Substrate, scales = "free_y") +
...
ggsave('fig2-1col.pdf', path = "./figures/", units = "mm", width = 89, height = 120, device = cairo_pdf, scale = 1.6)

```

#### Alpha over time stat test

Because n within each group is small (and sometimes missing reps), let's test for equal variance.
...
This set of graph shows alpha ~ Light only for during the day where there is light. (Many samples were taking during night, and these have been dropped.)
...
alpha.16s.light.only <- alpha.16s.light.only + 
  ...
  geom_jitter(width = 0) +
  facet_grid(facets = variable~Substrate, scales = "free_y") + #, switch = "y") +
  labs(title = "16S OTUs", x = expression("Irradiance ("~ Wm^{-2}~ ")")) +
  geom_text(data = df2, aes(x = graphx, y = graphy, label = eq), color = '#777777', parse = TRUE) +
...
alpha.18s.light.only <- alpha.18s.light.only + 
  ...
  geom_jitter(width = 0) +
  facet_grid(facets = variable~Substrate, scales = "free_y") +
  labs(title = "18S OTUs", x = expression("Irradiance ("~ Wm^{-2}~ ")")) +
  geom_text(data = df2, aes(x = graphx, y = graphy, label = eq), color = '#777777', parse = TRUE) +
  theme(legend.position = "right",
        strip.background = element_blank(),
        axis.title.y = element_blank()
...
ggsave('fig3-1col.pdf', path = "./figures/", units = "mm", width = 89, height = 120, device = cairo_pdf, scale = 1.6)
...
df2 <- data.frame(eq = c(eqns),
                  ...
                  graphy = c(400, 393, 400, .0226, .022, .022))
#df2
...
alpha.16s.1 <- alpha.16s.1 + 
  ...
  geom_smooth(method = "lm", color = viridis(n = 10)[8]) +
  geom_jitter(width = 0) +
  facet_grid(facets = variable~Substrate, scales = "free") +
  labs(title = "16S OTUs", x = expression("Productivity: " ~ Delta~delta^13~C / Delta ~t ~"  (‰"~ h^-1~")")) +
  #v.c + # also add color = Time..H.num to aes() to use this
  geom_text(data = df2, aes(x = graphx, y = graphy, label = eq), color = '#777777', parse = TRUE) +
  theme(legend.position = "right",
        strip.background = element_blank(),
...
df2 <- data.frame(eq = c(eqns),
                  ...
                  graphy = c(500, 650, 500, .025, .025, .025))
#df2
...
alpha.18s.1 <- alpha.18s.1 + 
  ...
  geom_jitter(width = 0) +
  facet_grid(facets = variable~Substrate, scales = "free") +
  labs(title = "18S OTUs", x = expression("Productivity: " ~ Delta~delta^13~C / Delta ~t ~"  (‰"~ h^-1~")")) +
  #v.c + # also add color = Time..H.num to aes() to use this
  geom_text(data = df2, aes(x = graphx, y = graphy, label = eq), color = '#777777', parse = TRUE) +
  theme(legend.position = "right",
        strip.background = element_blank(),
...
ggsave('fig4-1col.pdf', path = "./figures/", units = "mm", width = 89, height = 120, device = cairo_pdf, scale = 1.6)
...
...where made, but are not included here.
...
The same RA graphs used in the barplots will be used _before_ the merging of
samples. Here, we regenerate these from the rarefied  cohorts.
...
# Two alternative ordinations. These show that choise of carbon variable does not change ordination much
#cap.16s = ordinate(rar.16s.w_tax, "CAP", dist.16s,
#                   ~ Time..H. + SOLR.W.m.m + 
...
plot <- plot_ordination(rar.16s.w_tax, cap.16s, "samples", shape = "Substrate")
plot <- plot + geom_point(color = "lightgrey") +
	geom_path(aes(x = x, y=y, shape = NULL), data = circleFun(c(0,0), 1, 100), color="lightgrey") + 
...
plot_grid(plot, plot.18s,
          labels = c("A", "B"),
          align = "h", nrow = 1, rel_widths = c(1, 1.25))

ggsave('fig5-2col.pdf', path = "./figures/", units = "mm", width = 183, height = 84, device = cairo_pdf, scale = 1.3)
