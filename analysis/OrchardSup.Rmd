...
access the uniformity of the mat and our technical replicates.
...
The question relates to this analysis:
...
We can explore this several ways
1) Exploring time zero samples from "Native Mat" Samples
2) Compare variation between our technical reps and biologically different samples
3) Look at initial timepoints (h = 6) (imperfect, as substrate had already been added)
...
> I think we need to look at the beta diversity of the mat by only considering the earliest time points from each treatment (and/or whole experiment). My thought is that if there are not distinct clusters corresponding to each treatment then we can conclude that the microbiome is relatively homogenous between microcosms. 

```{r beta-variation-t1}
rar.16s.t1 <- subset_samples(rar.16s.cohort, timepoint %in% c("T1", "T6"))
bray.16s.t1 = ordinate(rar.16s.t1, method = "PCoA", distance = "bray")
plot_ordination(rar.16s.t1, bray.16s.t1, color="Substrate", shape = "timepoint") + geom_line()

rar.18s.t1 <- subset_samples(rar.18s.cohort, timepoint %in% c("T1", "T6"))
bray.18s.t1 = ordinate(rar.18s.t1, method = "PCoA", distance = "bray")
plot_ordination(rar.18s.t1, bray.18s.t1, color="Substrate", shape = "timepoint") + geom_line()

```

So after 6 hours, a treatment effect is already showing in these samples. I think the second method makes the most sense.
...
Reviewer's comment:
> L142: Please also provide that Shannon diversity index and it is more widespread...
